...
function submitleavel() {
    var url = '/dashboard/server_fun_categ/';
...
function submitleave2(currvalue) {
    ...
    var url = '/dashboard/server_app_categ/?fun_categId=' + currvalue;
    var myAjax = new Ajax.Request(
        url,
        {
            method: 'get',
...
function submitleave3(currvalue) {
    ...
    var url = '/dashboard/server_list/?app_categId=' + currvalue;
    //url=EncodeUtf8(url);
    var myAjax = new Ajax.Request(
        url,
        {
...
function submitmodule() {
    ...
    if (document.getElementById("sys_param_1")) {
        if (!checkform(1))
            return false;
        module_post_data['sys_param_1'] = $F('sys_param_1');
    }
    if (document.getElementById("sys_param_2")) {
        if (!checkform(2))
            return false;
        module_post_data['sys_param_2'] = $F('sys_param_2');
    ...
    var url = '/dashboard/module_run/';

    module_post_data['ModuleID'] = $F('modules');
    module_post_data['hosts'] = $F('serverApplist');
    var json_obj = Object.toJSON(module_post_data);
...
function submitmodule_add() {
    ...
    var module_name = $F('module_name')
    var module_caption = $F('module_caption')
    var module_extend = $F('module_extend')

    var url = '/dashboard/module_add/';
    var data = {'module_name': module_name, 'module_caption': module_caption, 'module_extend': module_extend};
    var myAjax = new Ajax.Request(
        url,
        {
            method: 'post',
...
function loadmoduleinfo(currvalue) {
    if (currvalue == -1)
        return false;
    var url = '/dashboard/module_info/';
    var module_info_data = {'currvalue': currvalue}
    var myAjax = new Ajax.Request(
        url,
        {
            method: 'post',
            parameters: module_info_data,
            onComplete: showResponse
        });

    function showResponse(originalRequest) {
        var objarray = originalRequest.responseText.evalJSON();

        var module_id = objarray['module_id'];  //module id
        var module_name = objarray['module_name'];  //module name
        var module_caption = objarray['module_caption'];  //module caption
        var module_extend = objarray['module_extend'];  //module extend
